...
import com.example.poadevice.domain.CoordinatesDto;
...
public class DeviceController {
    ...
    /**
     * @return URI that can be used to set the location of the launched service provider.
     * @throws IOException
     */
    @PostMapping(path = "/provide-location", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public String provideLocation(@RequestBody final CoordinatesDto coordinates) throws IOException {
        ...
        locationProviderProcess =
                new ProcessBuilder(
                        "java",
                        "-Dserver.ssl.key-store=file:" + CERTIFICATE_FILE,
                        "-Ddevice-longitude=" + coordinates.getLongitude(),
                        "-Ddevice-latitude=" + coordinates.getLatitude(),
                        "-jar", LOCATION_PROVIDER_JAR)
                                .inheritIO()
                                .start();
        return "OK";
    }
